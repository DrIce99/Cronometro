...
async function mostraClassifica() {
    ...
    sessioniFiltrate.forEach((g, index) => {
        ...
        if(editMode){
            editButtons = `
                <button class="subbtn-edit" onclick="openEditCard('${g.id}')">
...
// Apri card per modifica
function openEditCard(idGara) {
    // Trova la gara nel database locale
    const gara = dbLocale.find(g => g.id == idGara);
    if(!gara) return;

    currentEditGara = JSON.parse(JSON.stringify(gara));

    const body = document.getElementById('editBody');
    body.innerHTML = "";

    // Campi principali
    ['pilota','percorso','tipo','meteo'].forEach(f => {
        const input = document.createElement('input');
        input.value = currentEditGara[f] || '';
        input.dataset.field = f;
        body.appendChild(input);
    });

    // Parziali
    currentEditGara.parziali?.forEach((p, idx) => addParzialeRow(body, p, idx));

    // Pulsante per aggiungere un settore
    const addBtn = document.createElement('button');
    addBtn.textContent = "Aggiungi settore";
    addBtn.className = 'btn-add';
    addBtn.onclick = () => addParzialeRow(body, {tempo:'', ms:0}, body.querySelectorAll('.parziale-row').length);
    body.appendChild(addBtn);

    document.getElementById('editCard').classList.remove('hidden');
}

// Crea una riga parziale
function addParzialeRow(container, parziale, idx) {
    const row = document.createElement('div');
    row.className = 'parziale-row';

    const inputTempo = document.createElement('input');
    inputTempo.value = parziale.tempo || '';
    inputTempo.placeholder = "MM:SS.D";
    inputTempo.dataset.index = idx;
    inputTempo.dataset.field = 'tempo';
    row.appendChild(inputTempo);

    const inputMs = document.createElement('input');
    inputMs.type = "number"; // Assicuriamoci che sia un numero
    inputMs.value = parziale.ms != null ? parziale.ms : 0;
    inputMs.dataset.index = idx;
    inputMs.dataset.field = 'ms';
    row.appendChild(inputMs);

    // --- LOGICA DI RICALCOLO AUTOMATICO ---
    inputTempo.oninput = () => {
        const msCalcolati = timeToMs(inputTempo.value);
        if (msCalcolati !== null) {
            inputMs.value = msCalcolati;
            inputTempo.style.border = "2px solid #0f0"; // Verde se formato OK
        } else {
            inputTempo.style.border = "2px solid #f00"; // Rosso se formato errato
        }
    };

    inputMs.oninput = () => {
        const msVal = parseInt(inputMs.value) || 0;
        inputTempo.value = formatTime(msVal); 
    };
...
document.getElementById('btnSaveEdit').onclick = async () => {
    ...
    // Copia profonda della gara originale per confronto
    const originalGara = JSON.parse(JSON.stringify(currentEditGara));

    // Aggiorna campi principali e evidenzia modifiche
    inputs.forEach(inp => {
        const field = inp.dataset.field;
        if (!['tempo','ms'].includes(field)) {
            if (currentEditGara[field] !== inp.value) {
                inp.style.border = "2px solid #0f0"; // bordo verde se modificato
            } else {
                inp.style.border = "";
            }
            currentEditGara[field] = inp.value;
        }
    });

    // Raccolta parziali e confronto
    const parzialiRows = body.querySelectorAll('.parziale-row');
    currentEditGara.parziali = [];
    parzialiRows.forEach(row => {
        const tempoInput = row.querySelector('input[data-field="tempo"]');
        const msInput = row.querySelector('input[data-field="ms"]');
        
        const tempoVal = tempoInput.value || '';
        const msVal = parseInt(msInput.value) || 0; // <- sempre numero

        // Evidenzia se cambiato
        if (!originalGara.parziali?.[tempoInput.dataset.index] || 
            originalGara.parziali[tempoInput.dataset.index].tempo !== tempoVal ||
            originalGara.parziali[tempoInput.dataset.index].ms !== msVal) {
            tempoInput.style.border = "2px solid #0f0";
            msInput.style.border = "2px solid #0f0";
        } else {
            tempoInput.style.border = "";
            msInput.style.border = "";
        }

        currentEditGara.parziali.push({ tempo: tempoVal, ms: msVal });
    });

    // Invia PATCH sicuro
    try {
        const idPulito = parseInt(currentEditGara.id);
        const res = await fetch(`/api/update/${idPulito}`, {
            method: 'PATCH',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify(currentEditGara)
        });
        if (!res.ok) throw new Error("Errore server: " + res.status);
    } catch (e) {
        console.error("Errore aggiornamento:", e);
        alert("Errore aggiornamento server. Controlla console.");
        return;
    }

    document.getElementById('editCard').classList.add('hidden');
    mostraClassifica(); // ricarica lista
}
...
async function salvaModificheGara() {
    const body = document.getElementById('editBody');
    const inputs = body.querySelectorAll('input');

    // Copia profonda dell'oggetto
    const nuovoGara = JSON.parse(JSON.stringify(currentEditGara));

    // Aggiorna campi principali
    inputs.forEach(inp => {
        if (!['tempo', 'ms'].includes(inp.dataset.field)) {
            nuovoGara[inp.dataset.field] = inp.value;
        }
    });
    ...
    parzialiRows.forEach(row => {
        const tempoInput = row.querySelector('input[data-field="tempo"]');
        const msInput = row.querySelector('input[data-field="ms"]');
        const tempo = tempoInput.value || "00:00.0";
        const ms = parseInt(msInput.value) || 0;
        nuovoGara.parziali.push({tempo, ms});
    });

    try {
        const res = await fetch(`/api/update/${nuovoGara.id}`, {
            method: 'PATCH',
            headers: {'Content-Type':'application/json'},
            body: JSON.stringify(nuovoGara)
        });
        if(!res.ok) {
            const text = await res.text();
            console.error("Errore server:", text);
            alert("Errore aggiornamento: " + text);
            return;
        }
    } catch(e) {
    ...
    body.querySelectorAll('input').forEach(inp => {
        inp.addEventListener('input', async () => {
            if(inp.value != (currentEditGara[inp.dataset.field] ?? '')) {
                inp.classList.add('modified');
            } else {
                inp.classList.remove('modified');
            }
    
            await fetch(`/api/update/${currentEditGara.id}`, {
                method:'PATCH',
                headers:{'Content-Type':'application/json'},
                body: JSON.stringify(currentEditGara)
    ...
    parzialiRows.forEach((row, idx) => {
        const msInput = row.querySelector('input[data-field="ms"]');
        msInput.addEventListener('input', () => {
            const msVal = parseInt(msInput.value) || 0;
            const prevMs = idx > 0 ? parseInt(parzialiRows[idx-1].querySelector('input[data-field="ms"]').value) || 0 : 0;
            const delta = (msVal - prevMs) / 1000;
            const tempoInput = row.querySelector('input[data-field="tempo"]');
            tempoInput.value = formatTime(msVal - prevMs);
        });
    });
}


document.getElementById('btnSaveEdit').onclick = salvaModificheGara;
...
function timeToMs(timeStr) {
    ...
    const [_, min, sec, dec] = match;
    return (parseInt(min) * 60000) + (parseInt(sec) * 1000) + (parseInt(dec) * 100);
}